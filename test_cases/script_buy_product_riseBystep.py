...
class BuyProductScriptRiseByStep(InterfaceCase):
    ...
    def test_buy_riseBystep(self):
        '''购买步步高脚本，和buy_prodouct_info.xlsx数据源对应'''

        user_phone = self.inital.buyProduct_info['riseBystep']['phone']
        pwd = self.inital.buyProduct_info['riseBystep']['pwd']
        amount = self.inital.buyProduct_info['riseBystep']['amount']
        times = self.inital.buyProduct_info['riseBystep']['times']
        exec_flag = self.inital.buyProduct_info['riseBystep']['exec']
        load_create = self.inital.buyProduct_info['riseBystep']['load_create']

        if amount=="" or times=="" or exec_flag=="" or load_create=="":
            self.logger.info("BuyProductScriptRiseByStep.test_buy_riseBystep: buy_info excel parms is wrong!")
            return "buy_info excel parms is wrong!"


        elif exec_flag=='N' or times=='0':
            self.logger.info('BuyProductScriptRiseByStep.test_buy_riseBystep: do not buy product riseBystep!')
            return "do not buy product riseBystep!"
        ...
        if load_create=='N':
            ...
            self.logger.info("BuyProductScriptRiseByStep.test_buy_riseBystep: user_phone pwd value can not be null!")
            return  "BuyProductScriptRiseByStep.test_buy_riseBystep: user_phone pwd value can not be null!"
        ...
        for i in xrange(int(times)):
            try:
                ...
                self.logger.info("BuyProductScriptRiseByStep.test_buy_riseBystep:run {one} time;"
                                 "phone is {two};"
                                 "pwd is {three};"
                                 "product is {four};"
                                 "amount is {five}".format(one=(i+1),two=user_phone,three=pwd,four=sql_productNo,five=sql_amount))
            except Exception as e:
                self.logger.info("test_buy_riseBystep failed!"+str(e))
            finally:
                self.driver.quit()
